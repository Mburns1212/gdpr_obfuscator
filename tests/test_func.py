...
def s3_client_with_bucket_with_objects(s3_client_with_bucket):
    with open("tests/test_files/test.csv", "rb") as file:
        s3_client_with_bucket.put_object(
            Bucket="test-bucket", Key="test.csv", Body=file.read()
        )
    with open("tests/test_files/test_obj.json", "rb") as file:
        s3_client_with_bucket.put_object(
            Bucket="test-bucket", Key="test_obj.json", Body=file.read()
        )
    with open("tests/test_files/test_array.json", "rb") as file:
        s3_client_with_bucket.put_object(
            Bucket="test-bucket", Key="test_array.json", Body=file.read()
        )
    with open("tests/test_files/test_nested.json", "rb") as file:
        s3_client_with_bucket.put_object(
            Bucket="test-bucket", Key="test_nested.json", Body=file.read()
        )
    with open("tests/test_files/test.parquet", "rb") as file:
        s3_client_with_bucket.put_object(
            Bucket="test-bucket", Key="test.parquet", Body=file.read()
        )
...
def test_returns_bytestream_json(
    ...
    assert (
        type(
            obfuscator(
                {
                    "file_to_obfuscate": "s3://test-bucket/test_array.json",
                    "pii_fields": ["student_id"],
                }
            )
        )
...
def test_returns_bytestream_parquet(
    ...
    assert (
        type(
            obfuscator(
                {
                    "file_to_obfuscate": "s3://test-bucket/test.parquet",
                    "pii_fields": ["student_id"],
                }
            )
        )
...
def test_obfuscates_piis_json_obj(
    ...
    result = obfuscator(
        {
            "file_to_obfuscate": "s3://test-bucket/test_obj.json",
            "pii_fields": ["student_id"],
        }
    ).decode("utf-8")
...
def test_obfuscates_piis_nested_json(
    ...
    result = obfuscator(
        {
            "file_to_obfuscate": "s3://test-bucket/test_nested.json",
            "pii_fields": ["name"],
        }
    ).decode("utf-8")
...
def test_invalid_file_type(mock_client, s3_client_with_bucket_with_objects):
    ...
    with pytest.raises(ValueError, match="txt files are not supported"):
        obfuscator(
            {
                "file_to_obfuscate": "s3://test-bucket/test.txt",
                "pii_fields": ["student_id"],
            }
        )
...
def test_input_missing_keys(mock_client, s3_client_with_bucket_with_objects):
    mock_client.return_value = s3_client_with_bucket_with_objects
    with pytest.raises(
        ValueError, match=re.escape("Missing key(s): {'pii_fields'}")
    ):
